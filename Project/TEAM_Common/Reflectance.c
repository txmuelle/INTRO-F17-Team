...
#define REF_SENSOR_TIMEOUT_US  1500
	const RefCnt_TValueType timeoutCntVal = ((RefCnt_CNT_INP_FREQ_U_0 / 1000)
			* REF_SENSOR_TIMEOUT_US)
			/ 1000 /* REF_SENSOR_TIMEOUT_US translated into timeout ticks */;
...
static uint8_t PrintHelp(const CLS1_StdIOType *io) {
	...
	CLS1_SendHelpStr((unsigned char*) "  help|status",
			...
			io->stdOut);
#if REF_START_STOP_CALIB
	CLS1_SendHelpStr((unsigned char*) "  calib (start|stop)",
			(unsigned char*) "Start/Stop calibrating while moving sensor over line\r\n",
			io->stdOut);
#endif
	return ERR_OK;
}
...
#if 1 || PL_CONFIG_HAS_LINE_FOLLOW
	CLS1_SendStatusStr((unsigned char*) "  line kind",
			REF_LineKindStr(refLineKind), io->stdOut);
	CLS1_SendStr((unsigned char*) "\r\n", io->stdOut);
#endif
	return ERR_OK;
}
	...
	if (UTIL1_strcmp((char*)cmd, CLS1_CMD_HELP) == 0
		...
		*handled = TRUE;
		...
		if (refState == REF_STATE_NOT_CALIBRATED
			...
			CLS1_SendStr(
					(unsigned char*) "ERROR: cannot start calibration, must not be calibrating or be ready.\r\n",
					io->stdErr);
			return ERR_FAILED;
		}
		*handled = TRUE;
		...
		if (refState == REF_STATE_CALIBRATING) {
			...
			CLS1_SendStr(
					(unsigned char*) "ERROR: can only stop if calibrating.\r\n",
					io->stdErr);
			return ERR_FAILED;
		}
		*handled = TRUE;
...
#if PL_CONFIG_HAS_CONFIG_NVM
		if (NVMC_SaveReflectanceData(&SensorCalibMinMax,
				sizeof(SensorCalibMinMax)) != ERR_OK) {
			SHELL_SendString(
					(unsigned char*) "Flashing calibration data FAILED!\r\n");
		} else {
...
void REF_Init(void) {
	...
	vQueueAddToRegistry(REF_StartStopSem, "RefStartStopSem"); //old: FRTOS1_vQueueAddToRegistry(REF_StartStopSem, "RefStartStopSem");
#endif
